...
/**
 * FRONTEND PRIVATE MAIL v4.8.8 ELITE (TOTAL CLEAN BUILD)
 * Fokus: Resolusi TS6133 - Penghapusan total variabel tak terpakai.
 */
...
export default function App() {
  ...
  const formatBody = (rawBody: string) => {
    ...
    let clean = rawBody.replace(/(Content-Type|Content-Transfer-Encoding|charset|boundary|MIME-Version|Subject|From|To|Date|X-[\w-]+):.*?\r?\n/gi, '');
    clean = clean.replace(/--[a-f0-9]{10,}/gi, '');
    clean = clean.replace(/^[a-zA-Z0-9+/=]{60,}$/gm, ''); 
    clean = clean.replace(/\r\n/g, '\n').replace(/\n{3,}/g, '\n\n');
    return clean.trim() || "Pesan terenkripsi atau hanya berisi teks teknis.";
  };
  ...
  const checkApiHealth = useCallback(async () => {
    try {
      ...
      if (response.ok) setConnectionStatus('online');
    } catch {
  ...
  const fetchMessages = useCallback(async (manual = false) => {
    ...
    try {
      ...
      const targetUrl = `${baseUrl}/messages?email=${encodeURIComponent(cleanEmail)}&_r=${Date.now()}`;
      ...
      // Catch silent untuk menghindari unused variable 'err'
    } finally {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0a0a0c] text-zinc-400 font-sans grid place-items-center p-4 sm:p-8 overflow-hidden">
      ...
      <div className="relative w-full h-[88vh] max-w-5xl flex flex-col md:flex-row bg-[#111114] border border-white/[0.04] rounded-2xl shadow-2xl overflow-hidden transition-all duration-500">
        <aside className="w-full md:w-60 flex flex-col border-b md:border-b-0 md:border-r border-white/[0.04] bg-[#111114] shrink-0 text-left">
          <div className="p-6 flex flex-col h-full text-left">
            <div className="flex items-center gap-3 mb-10 text-left">
              <div className="p-2 bg-indigo-600 rounded-lg shadow-lg text-left">
                <ShieldCheck className="w-5 h-5 text-white" />
              </div>
              <h1 className="text-sm font-bold text-zinc-100 tracking-tight uppercase italic leading-none">PrivateMail</h1>
            </div>
            ...
            <div className="space-y-8 flex-grow text-left">
              <div className="space-y-4 text-left">
                ...
                <div className="bg-zinc-900/40 border border-white/[0.03] rounded-xl p-4 transition-all">
                  <div className="text-center">
                    <p className="text-[13px] font-mono text-indigo-400 font-bold truncate mb-4 text-center">{loading ? '...' : email}</p>
              ...
              <nav className="space-y-2 text-left">
                <div className="flex items-center justify-between px-3 py-3 bg-indigo-600/10 text-indigo-400 rounded-xl font-bold text-[11px] uppercase tracking-wide border border-indigo-600/20 text-left">
                  <div className="flex items-center gap-3 text-left"><Inbox className="w-4 h-4 text-left" /> Masuk</div>
                  <span className="text-[10px] bg-indigo-500 text-white px-2 py-0.5 rounded-md min-w-[1.5rem] text-center">{messages.length}</span>
                </div>
              </nav>
            </div>
        ...
        <section className={`w-full md:w-80 flex flex-col border-r border-white/[0.04] bg-[#141417] text-left ${selectedMessage ? 'hidden md:flex' : 'flex'}`}>
          <div className="p-5 border-b border-white/[0.04] flex items-center justify-between bg-[#141417]">
            ...
            <button onClick={() => fetchMessages(true)} className="p-1.5 hover:bg-zinc-800 rounded-lg transition-all text-zinc-500 hover:text-zinc-200">
          ...
          <div className="flex-grow overflow-y-auto custom-scrollbar text-left">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center p-10 text-center opacity-20 italic">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest leading-relaxed text-center">Menunggu paket...</p>
              </div>
            ) : (
              messages.map((msg) => (
                <div key={msg.id} onClick={() => setSelectedMessage(msg)} className={`p-5 border-b border-white/[0.02] cursor-pointer transition-all ${selectedMessage?.id === msg.id ? 'bg-indigo-600/[0.03] border-l-4 border-l-indigo-600' : 'hover:bg-zinc-800/30'} text-left`}>
                  ...
                  <p className="text-[11px] text-zinc-500 truncate font-medium italic opacity-70 text-left leading-none">{msg.subject || '(Tanpa Subjek)'}</p>
                </div>
              ))
            )}
          </div>
        ...
        <main className="flex-grow flex flex-col bg-[#111114] relative text-left">
          {showTerminal ? (
            <div className="flex flex-col h-full bg-[#09090b] p-8 animate-in fade-in duration-500 text-left text-left">
               <div className="flex items-center gap-3 mb-6 pb-4 border-b border-zinc-800 text-left">
                 ...
                 <h3 className="text-xs font-bold text-zinc-100 uppercase tracking-widest text-left">Diagnostik</h3>
               </div>
               <div className="flex-grow space-y-4 overflow-y-auto custom-scrollbar font-mono text-[11px] text-zinc-500 text-left">
                 <div className="p-4 bg-zinc-900/50 rounded-xl border border-zinc-800 text-left text-left">
                   <p className="text-indigo-400 font-bold mb-2 uppercase tracking-widest text-left">[OK] DATABASE & INFRASTRUKTUR</p>
                   <p className="italic text-left leading-relaxed">Pembersihan variabel tak terpakai selesai. Node siap transmisi.</p>
                 </div>
               </div>
               <button onClick={() => setShowTerminal(false)} className="mt-4 px-6 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg font-bold uppercase text-[10px] tracking-widest transition-all">Tutup</button>
            </div>
          ) : selectedMessage ? (
            <div className="flex flex-col h-full animate-in fade-in duration-300 text-left">
              <div className="p-6 border-b border-white/[0.04] flex items-center justify-between bg-[#141417]/60 backdrop-blur-md text-left">
                <div className="flex items-center gap-5 overflow-hidden text-left">
                  ...
                  <div className="w-11 h-11 bg-indigo-600 rounded-xl flex items-center justify-center font-bold text-white text-xl shrink-0 text-center">
                ...
                <button onClick={() => setSelectedMessage(null)} className="p-2.5 text-zinc-600 hover:text-red-400 hover:bg-red-400/5 rounded-xl transition-all">
                  <Trash2 className="w-4.5 h-4.5 text-center" />
                </button>
              </div>
              <div className="flex-grow overflow-y-auto p-8 md:p-12 custom-scrollbar bg-[#0d0d0f] text-left text-left">
                <div className="max-w-2xl mx-auto text-left">
                   <div className="flex items-center gap-3 mb-8 opacity-40 text-left text-left">
                      <Clock className="w-3.5 h-3.5 text-left" />
                      <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-left text-left">Diterima {new Date(selectedMessage.date).toLocaleString()}</span>
                   </div>
                   ...
                   <div className="mt-16 flex flex-col items-center opacity-10 text-center mx-auto text-center">
                      <div className="w-32 h-[1px] bg-zinc-700 mb-8 mx-auto text-center"></div>
            ...
            <div className="h-full flex flex-col items-center justify-center p-12 text-center animate-in zoom-in-95 duration-500 text-center">
              <div className="relative mb-8 text-center mx-auto text-center">
                ...
                <div className="relative w-24 h-24 bg-zinc-900/80 rounded-[2.5rem] flex items-center justify-center border border-white/[0.04] shadow-2xl mx-auto text-center">
                   <Mail className="w-10 h-10 text-zinc-700 text-center mx-auto text-center" />
                </div>
              </div>
              <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-[0.4em] mb-4 text-center text-center">Standby</h3>
              <p className="text-[12px] max-w-[280px] leading-relaxed text-zinc-600 font-medium italic opacity-60 mx-auto text-center">
          ...
          <div className="px-6 py-3.5 bg-[#141417] border-t border-white/[0.04] flex items-center justify-between gap-4 text-left">
             <div className="flex items-center gap-3 text-left">
                ...
                <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest italic text-left text-left text-left text-left">Node: {MY_DOMAIN}</span>
             </div>
             <div className="flex items-center gap-3 opacity-40 text-left">
                <Shield className="w-3.5 h-3.5 text-indigo-400 text-left text-left" />
                <span className="text-[9px] font-black uppercase tracking-widest text-left text-left">RSA-4096 Secure</span>
             </div>
          </div>
        </main>
      </div>
